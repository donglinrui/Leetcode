...
class Solution:
    def maxProfit(self, prices: List[int]) -> int:
        ...
        probit_max = 0 
        for p in prices:
            price_min = min(price_min,p)#记录p之前的最小的price，用于计算porbit
            probit_max = max(probit_max,p-price_min)#最大的利润，当前卖出的probit和之前的最大值
        return probit_max
```
...
首尾相接且不能相邻，则分为有首没尾和有尾没首两种情况考虑问题。
